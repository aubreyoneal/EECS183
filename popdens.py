...
def advanceYears(data, years):
    rows = len(data)
    for i in range(0,rows): #goes through each country
        futurePop = float((data[i][3]**years) * data[i][2])
        data[i].insert(4, futurePop)
    for i in range(0,rows):
        futureDens = float(futurePop / data[i][1])
...
def histogramLine(name, dens):
    ...
    density = range(0, int(math.ceil(dens/1000)))
...
def displayTable(data):
    ...
    totalPop = 0
    for i in row:
        print '{0:35s}{1:>15.0f}{2:>15.1f}'.format(data[i][0], data[i][4], data[i][5])
        totalPop += data[i][4]
    for count in line:
        print '-',; sys.stdout.softspace = False
    print
...
def main(argv):
    ...
    data = processInputFile(filename)
    future = advanceYears(data, 3)
    sortByDensity(future)
